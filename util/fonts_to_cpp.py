...
ignore:list[re.compile] = [
    re.compile(r'^u8g2_.*?japanese'),
    re.compile(r'^u8g2_.*?unifont.*?(chinese|korean)'),
    re.compile(r'^u8g2_.*?streamline_all'),
    re.compile(r'^u8g2_font_t0_11_t_all'),
]
...
with (open(parent / "Fonts.txt") as f,
    ...
    for line in f:
        ...
        if not result:
            ...
            continue
        isIgnored = False
        for j, r in enumerate(ignore):
            if r.match(fontname):
                isIgnored = True
                ignored += 1
                break
        matched += 1
        fonts[result['name'] or ""][result['size'] or ""][result['alt'] or ""][result['alpha'] or ""][result['face'] or ""] = (fontname, i, isIgnored, j)
...
""")
                            ...
                            for alpha, faces in sorted(alphas.items()):
                                ...
                                with Namespace(h, alpha, "   "):
                                    for face, result in sorted(faces.items()):
                                        u8g2Name, reIndex, isIgnored, igIndex = result
                                        face = fixCname(face)
                                        cname = fontname
                                        fullcname = "Fonts"
                                        if fontname:
                                        ...
                                        if face:
                                            ...
                                            fullcname += "::" + face
                                        h.write(f"    // {u8g2Name}\n")
                                        prefix = "// " if isIgnored else ""
                                        suffix = f" ignored by {igIndex}" if isIgnored else ""
                                        h.write(f"    {prefix}extern const uint8_t* {cname};{suffix}\n")
                                        if isIgnored:
                                            suffix = ";" + suffix
                                        cppLines.append((isIgnored, fullcname, u8g2Name+";", f"// matched by re #{reIndex+1}{suffix}"))
    # write to cpp
    maxName = 0
    maxAssign = 0
    for l in cppLines:
        if len(l[1]) > maxName:
            maxName = len(l[1])
        if len(l[2]) > maxAssign:
            maxAssign = len(l[2])
    for l in cppLines:
        cname = l[1]

        cname = "const uint8_t* "+cname
        if l[0]:
            cname = "// " + cname
        cname = cname.ljust(maxName+3)
        cpp.write(f"{cname} = {l[2].ljust(maxAssign)} {l[3]}\n")
    ...
    print(f'Matched {matched} fonts; {len(notMatched)} not matched.; {ignored} ignored.\n')
